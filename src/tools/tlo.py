"""TLO/TransUnion investigative data (requires security firm verification)."""
...
@register_tool
class TloTool(BaseTool):
    name = "tlo"
    description = "TLO/TransUnion investigative data (requires security firm verification)"
    binary_name = "tlo"
    install_cmd = "Contact vendor: https://www.tlo.com"
    accepted_target_types = (TargetType.PHONE, TargetType.EMAIL, TargetType.PERSON_NAME)
    requires_api_keys = ("tlo.api_key",)
    requires_fcra = True
    estimated_cost_per_query = 0.0

    def is_installed(self) -> bool:
        return bool(self.config.get_api_key("tlo", "api_key"))
