"""Tracers investigative data platform (requires licensed investigator credentials)."""
...
@register_tool
class TracersTool(BaseTool):
    name = "tracers"
    description = "Tracers investigative data platform (requires licensed investigator credentials)"
    binary_name = "tracers"
    install_cmd = "Contact vendor: https://www.tracers.com"
    accepted_target_types = (TargetType.PHONE, TargetType.EMAIL, TargetType.PERSON_NAME)
    requires_api_keys = ("tracers.api_key",)
    requires_fcra = True
    estimated_cost_per_query = 0.0

    def is_installed(self) -> bool:
        return bool(self.config.get_api_key("tracers", "api_key"))
        ...
        if not self.is_installed():
            return ToolResult(
                ...
                errors=[
                    ...
                    f"Contact: https://www.tracers.com"
                ],
            )
        # If API key is configured, the user has negotiated access.
        # Implementation depends on their specific contract/endpoint.
        api_key = self.config.get_api_key("tracers", "api_key")
        endpoint = self.config.get_api_key("tracers", "api_endpoint") or ""
        return ToolResult(
            tool_name=self.name, target=target, raw_output="",
            structured_data={"findings": []}, execution_time_seconds=0.0,
            errors=[
                f"{self.name} API key is configured but no vendor-specific endpoint is implemented. "
                f"Set tracers.api_endpoint in api_keys.yaml to your contract-specific URL."
            ],
        )
